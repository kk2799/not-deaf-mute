...
Extends v2 (which adapted attention only, mel 0.594):
  ...
  4. regime name = "lora_v3b" → separate results.csv rows.

Baseline comparison (v2 to beat):
  mel:   v1 0.578 / v2 0.594 / probe 0.675 → v3 target ≥ 0.61

Usage: python scripts/run_lora_vl8b_v3b.py --specs mel
"""
...
LORA_DIR = Path("outputs/lora_v3b")
SEED = 42
...
def main():
    ...
    if args.specs == "mel,stft,demon" and not args.limit:
        (RESULTS / "lora_v3b_vl8b.done").write_text("done\n")
