#!/usr/bin/env python
"""LoRA v2: VL-8B with BOTH language model AND vision tower adapters.

Improvements over run_lora_vl8b.py (v1):
  1. Vision tower LoRA — v1 only adapted language model attention, leaving the
     spectrogram-feature extractor frozen. The Qwen3-VL vision tower (27 blocks,
     hidden 1152) uses fused qkv + proj (NOT separate q/k/v_proj like the LLM).
  2. rank 32 (v1 used 8) — more adapter capacity.
  3. regime name = "lora_v2" → separate results.csv rows, does NOT overwrite v1.

Baseline comparison (v1 results to beat):
  mel:   lora_ft = 0.578 / probe = 0.675 → v2 target ≥ 0.62
  stft:  lora_ft = 0.558 / probe = 0.659 → v2 target ≥ 0.60
  demon: lora_ft = 0.446 / probe = 0.483 → v2 target ≥ 0.46

Usage: python scripts/run_lora_vl8b_v2.py --specs mel   # quick validation first
"""
...
# v2: language model attention + vision tower attention (fused qkv + proj)
LORA_TARGET = (
...
def main():
    ...
    for spec in args.specs.split(","):
        ...
        print(f"\n=== eval {exp} (vision+lang LoRA r{args.rank}) ===")
    ...
    if args.specs == "mel,stft,demon" and not args.limit:
        (RESULTS / "lora_v2_vl8b.done").write_text("done\n")
    print("\n✅ LoRA v2 (vision+lang) complete")
